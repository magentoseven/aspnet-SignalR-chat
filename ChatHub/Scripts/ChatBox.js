...
var updateChatLog = function (chatlog) {
    ...
    $("#chatscrollpanel").animate({ scrollTop: $('#chatscrollpanel').prop("scrollHeight") }, 500);
}

// Get chat log from Web API.
var getChatLog = function () {
    console.log("ChatHub: Getting chat log...");
    $.ajax({
        url: "/api/Chat",
        type: "GET",
...
var getChatWindowState = function () {
    var chatWindowState = "OFF";

    if ($.cookie) {
        $.cookie('ChatHub.ChatWindowState');
        if (!chatWindowState) {
            chatWindowState = "OFF";
            $.cookie('ChatHub.ChatWindowState', chatWindowState, { expires: 1, path: '/' });
        } else {
            chatWindowState = $.cookie('ChatHub.ChatWindowState');
        }
    } else {
        console.log("ChatHub : Cookie is not supported!");
        chatWindowState = $("#ChatWindowState").val();
    }
...
var setChatWindowState = function (chatWindowState) {
    if ($.cookie) {
        ...
        $("#ChatWindowState").val(chatWindowState);
    }
}
...
$(function () {
    if ($.connection) {
        console.log("ChatHub: Connection is available.");
        ...
        console.log("ChatHub : No Connection!");
    }
